  ...
  if (len > 0) {
    for (var i = 0; i < len; i++) {
      if (result[i].coursecode && result[i].task && result[i].deadline && result[i].weight) {
        ...
        rawpriority = (result[i].weight / (jsDateToExcelDate(dueDate) - jsDateToExcelDate(rightNow))) * 100;
        priority = parseFloat(rawpriority).toFixed(2);
        if (rawpriority < 0.0000001 || result[i].completed) {
          isCompleted = true;
        } else {
          isCompleted = false;
        }
